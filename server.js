...
async function generateReplyForTicket({ ticketId, tone, mode = "preview", webhookEvent = null, includeDeepContext = true, extraEmails = [], autoUpdateTicket = false, dryRunTicketUpdate = true, categoryId = null, categoryName = null }) {
  ...
  const ticketActionHints = await getTicketActionHints({ ticket: context.ticket, ai });

  let ticketAutoUpdateResult = null;
  if (arguments[0] && arguments[0].autoUpdateTicket) {
    ticketAutoUpdateResult = await applyTicketCloseAndCategory({
      ticketId,
      ai,
      ticket: context.ticket,
      categoryId: arguments[0].categoryId || null,
      categoryName: arguments[0].categoryName || null,
      dryRun: arguments[0].dryRunTicketUpdate !== false
    });
  }
...
async function getTicketActionHints({ ticket, ai }) {
  const dictionaries = await getTicketDictionaries();
  ...
  const category = guessTicketCategory({ ai, ticket, categories });
...
async function applyTicketCloseAndCategory({ ticketId, ai, ticket, categoryId = null, categoryName = null, dryRun = false }) {
  const hints = await getTicketActionHints({ ticket, ai });
  ...
  let finalCategory = hints.category;

  if (categoryId) {
    finalCategory = { id: categoryId, name: categoryName || String(categoryId), raw: { id: categoryId } };
  } else if (categoryName) {
    const categories = extractDictionaryData(hints.dictionaries, "categories");
    const found = findItemByName(categories, categoryName);
    if (found) {
      finalCategory = {
        id: itemId(found),
        name: itemDisplayName(found),
        raw: found
      };
    }
  }

  if (finalCategory && finalCategory.id !== null) {
    update.category_id = finalCategory.id;
    update.ticket_category_id = finalCategory.id;
  }
...
async function sendFinalTicketResponse({ ticketId, content, categoryId = null, categoryName = null, closeTicket = true, dryRun = false }) {
  ...
  const actionResult = closeTicket
    ? await applyTicketCloseAndCategory({ ticketId, ai: fakeAi, ticket, categoryId, categoryName, dryRun })
  ...
  if (dryRun) {
    return {
      ...
      content,
      actionResult,
      message: "Dry-run wysyłki. Ustaw dryRun=false oraz THULIUM_TICKET_WRITE_ENABLED=true, aby wysłać."
    };
  }

  const sendResult = await addTicketAgentResponse(ticketId, content);
...
async function getTicketDictionaries() {
  const client = createThuliumClient();

  const endpoints = [
    { key: "statuses", path: "/api/ticket_statuses" },
    { key: "queues", path: "/api/ticket_queues" },
    { key: "categories", path: "/api/ticket_categories" }
  ];
...
async function getTicketTodayStats() {
  const client = createThuliumClient();

  try {
    const response = await client.get("/api/tickets/today_stats");
...
async function searchTicketsAdvanced({ email, text, status, queueId, limit = 25 }) {
  const client = createThuliumClient();
  ...
  for (const value of queryValues) {
    attempts.push({ query: value });
    attempts.push({ q: value });
    attempts.push({ search: value });
    if (extractEmail(value)) {
      attempts.push({ email: value });
      attempts.push({ customer_email: value });
      attempts.push({ requester_email: value });
    }
  }
  ...
  for (const row of results) {
    ...
    seen.add(id);
    unique.push(row);
    if (unique.length >= Number(limit || 25)) break;
  }
...
function isWriteAllowed(actionName) {
  ...
  if (!enabled) {
    return {
      ...
      message: `Write action ${actionName} is disabled. Set THULIUM_TICKET_WRITE_ENABLED=true in Render to enable.`
    };
  }
  return { ok: true };
}

async function updateTicket(ticketId, updatePayload) {
  const allowed = isWriteAllowed("updateTicket");
  ...
  for (const payload of payloadCandidates) {
    try {
      const response = await client.put(`/api/tickets/${encodeURIComponent(ticketId)}`, payload);
      return { ok: true, usedPayload: payload, data: response.data };
    } catch (error) {
      lastError = error;
      const status = error.response && error.response.status;
  ...
  throw lastError || new Error("Could not update ticket");
}

async function addTicketAgentResponse(ticketId, content) {
  const allowed = isWriteAllowed("addTicketAgentResponse");
  if (!allowed.ok) return allowed;

  return sendAgentResponse(ticketId, content);
}

async function addTicketCustomerResponse(ticketId, content) {
  const allowed = isWriteAllowed("addTicketCustomerResponse");
  if (!allowed.ok) return allowed;
  ...
  for (const payload of payloadCandidates) {
    try {
      const response = await client.post(`/api/tickets/${encodeURIComponent(ticketId)}/customer_response`, payload);
      return { ok: true, usedPayload: payload, data: response.data };
    } catch (error) {
      lastError = error;
      const status = error.response && error.response.status;
  ...
  throw lastError || new Error("Could not add customer response");
}

async function safeTicketComment(ticketId, content) {
  const allowed = isWriteAllowed("addTicketComment");
  if (!allowed.ok) return allowed;

  return addTicketComment(ticketId, content);
}

function buildTicketDigest({ ticket, customer, deepContextSummary }) {
  const ticketJson = JSON.stringify(ticket || {}, null, 2).slice(0, 12000);
  const customerJson = JSON.stringify(customer || {}, null, 2).slice(0, 6000);
  const deepJson = JSON.stringify(deepContextSummary || {}, null, 2).slice(0, 8000);

  return [
    "PODSUMOWANIE KONTEKSTU TICKETU",
    "",
    "Ticket:",
    ticketJson,
    "",
...
app.post("/api/ticket-apply-close-category", requireExtensionKey, async (req, res) => {
  try {
    ...
    const result = await applyTicketCloseAndCategory({
      ticketId: String(ticketId),
      ai: fakeAi,
      ticket,
      categoryId,
