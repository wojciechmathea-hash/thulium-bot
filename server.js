...
try {
  ...
  console.warn("[Platforms] Playwright not available:", error.message);
}
...
- danych odczytanych wyłącznie w trybie read-only z platform EDU i VOD,
...
ZASADY KORZYSTANIA Z PLATFORM EDU/VOD:
- Dane z platform EDU i VOD są tylko kontekstem dla konsultanta.
- Agent nie może wykonywać żadnych zmian na platformach.
- Agent nie może twierdzić, że aktywował lub zmienił dostęp, chyba że wynika to z danych.
- Jeśli status dostępu jest niejasny, poproś konsultanta o ręczną weryfikację lub klienta o brakujące dane.
- Jeżeli klient pyta o dostęp do kursu/VOD, wykorzystaj odczytane dane o koncie, e-mailu, kursach i statusach dostępu.
...
- Jeśli nie znaleziono konta po e-mailu, poproś o e-mail użyty przy zakupie/rejestracji.
- Jeśli dane z platformy są niejednoznaczne lub nie udało się ich pobrać, oznacz sprawę jako wymagającą weryfikacji człowieka.
- Nie pisz klientowi szczegółów technicznych panelu administracyjnego.
...
async function collectPlatformContexts({ email, ticketText }) {
  if (String(process.env.PLATFORM_LOOKUP_ENABLED || "true").toLowerCase() !== "true") {
    return [];
  }

  const query = email || extractEmail(ticketText || "");
  if (!query) {
    return [{
      platform: "all",
      ok: false,
      skipped: true,
      reason: "No email found in ticket/customer context"
    }];
  }
  ...
  if (!configs.length) {
    return [{
      ...
      ok: false,
      skipped: true,
      reason: "No platform env variables configured"
    }];
  }
  ...
  for (const config of configs) {
    try {
      const result = await withTimeout(readPlatformSnapshot(config, query), timeoutMs, config.name);
      results.push(result);
    } catch (error) {
      results.push({
        platform: config.id,
...
async function readPlatformSnapshot(config, query) {
  if (!chromium) {
    return {
      platform: config.id,
      platformName: config.name,
...
async function clickLogin(page) {
  const buttonNames = ["Log in", "Login", "Zaloguj", "Sign in", "Submit"];
...
function buildAgentInput({ ticket, customer, tone, platformContexts }) {
  return {
    instruction: "Przygotuj propozycję odpowiedzi do klienta na podstawie danych z Thulium, odczytu platform EDU/VOD oraz zasad firmy.",
    tone,
...
async function generateReplySuggestion({ ticket, customer, tone, platformContexts }) {
  const openai = getOpenAIClient();
  const modelInput = buildAgentInput({ ticket, customer, tone, platformContexts });
...
async function generateReplyForTicket({ ticketId, tone, mode = "preview", webhookEvent = null }) {
  ...
  const platformContexts = await collectPlatformContexts({ email, ticketText });
  console.log(`[AI] Platform contexts loaded ticket=${ticketId}, count=${platformContexts.length}, email=${email || "none"}`);

  const ai = await generateReplySuggestion({ ticket, customer, tone, platformContexts });
  ...
  return { ticketId, ai, formattedComment, thuliumWriteResult, platformContexts };
}
...
app.post("/api/platform-context", requireExtensionKey, async (req, res) => {
  try {
    ...
    const contexts = await collectPlatformContexts({ email, ticketText: email });
    res.json({ ok: true, email, contexts });
  } catch (error) {
    console.error("Platform context failed:", normalizeError(error));
    res.status(500).json({
      ok: false,
      error: "PLATFORM_CONTEXT_FAILED",
...
app.post("/api/preview-prompt", requireExtensionKey, async (req, res) => {
  try {
    ...
    const customer = customerId ? await safeGetCustomer(customerId) : null;
    const email = extractLikelyCustomerEmail(ticket, customer);
    const platformContexts = await collectPlatformContexts({ email, ticketText: JSON.stringify({ ticket, customer }) });

    res.json({
      ok: true,
      ticketId: String(ticketId),
      email,
      promptPreview: buildAgentInput({ ticket, customer, tone, platformContexts })
    });
  } catch (error) {
...
app.post("/api/mock-reply", requireExtensionKey, async (req, res) => {
  ...
  const ai = {
    suggested_reply: "Dzień dobry,\n\nDziękujemy za wiadomość. Abyśmy mogli dokładnie zweryfikować sprawę, prosimy o przesłanie e-maila użytego przy zakupie lub rejestracji na platformie.\n\nPo otrzymaniu tych informacji sprawdzimy status dostępu i wrócimy z odpowiedzią.\n\nPozdrawiamy,\nZespół Obsługi Klienta",
  ...
  res.json({ ok: true, ticketId: String(ticketId), ai, formattedComment: formatAiComment(ai, { ticketId, mode }), thuliumWriteResult: null });
});
...
app.post("/api/generate-reply", requireExtensionKey, async (req, res) => {
  try {
    ...
    const result = await generateReplyForTicket({ ticketId: String(ticketId), tone, mode });
    res.json({ ok: true, ...result });
  } catch (error) {
