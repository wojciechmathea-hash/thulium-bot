...
function escapeHtmlForThuliumV097(value) {
  return String(value || "")
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;")
...
function textToHtmlForThuliumV097(value) {
  const escaped = escapeHtmlForThuliumV097(value);
  return escaped
    .split(/\n{2,}/)
    .map(part => `<p>${part.replace(/\n/g, "<br>")}</p>`)
    .join("");
...
function buildAgentResponsePayloadsV097(content) {
  const html = textToHtmlForThuliumV097(content);
...
function buildAgentResponseFormPayloadsV097(content) {
  const html = textToHtmlForThuliumV097(content);
  return [
    { key: "body[content]", value: content },
    { key: "body[text]", value: content },
    { key: "body[message]", value: content },
    { key: "body[value]", value: content },
    { key: "body[html]", value: html },
    { key: "body[0][content]", value: content },
    { key: "body[0][text]", value: content },
    { key: "body[0][html]", value: html },
    { key: "message[body]", value: content },
    { key: "message[body][content]", value: content },
    { key: "message[body][text]", value: content },
    { key: "message[content]", value: content },
    { key: "message[text]", value: content }
  ];
}
...
async function sendTicketReplySmart(ticketId, content) {
  ...
  const jsonPayloads = buildAgentResponsePayloadsV097(content);
  ...
  const formPayloads = buildAgentResponseFormPayloadsV097(content);
  ...
  return {
    ok: false,
    senderVersion: "v0.9.7-structured-body-only",
    error: "AGENT_RESPONSE_ENDPOINT_REJECTED_STRUCTURED_BODY_PAYLOADS",
    message: "Endpoint /agent_response odrzucił wszystkie strukturalne formaty body. Ticket NIE został zamknięty.",
    endpoint,
    firstAttemptMustBe: { body: { content } },
    attempts
  };
}
