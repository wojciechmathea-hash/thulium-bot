...
const VERSION = "1.0.0-audited";

/* AUDITED v1.0.0 - globally scoped Thulium ticket workflow.
   Verified endpoints from provided Thulium REST API PDF:
   ...
   PUT  /api/tickets/:id
*/
const AUDITED_THULIUM_TICKET_ENDPOINTS = Object.freeze({
...
function auditedWriteAllowed(actionName) {
...
function auditedArray(value) {
  if (Array.isArray(value)) return value;
  if (!value || typeof value !== "object") return [];
  for (const key of ["data", "items", "results", "rows", "records", "statuses", "categories", "queues"]) {
    if (Array.isArray(value[key])) return value[key];
  }
  return Object.values(value).filter(item => item && typeof item === "object");
}

function auditedId(item) {
  if (!item || typeof item !== "object") return null;
  return item.id ?? item.status_id ?? item.category_id ?? item.ticket_category_id ?? item.value ?? null;
}

function auditedName(item) {
  if (!item || typeof item !== "object") return "";
  return item.name || item.title || item.label || item.value || item.status || item.category || item.description || "";
}

function auditedNormalize(value) {
  return String(value || "")
    .toLowerCase()
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
...
function auditedFindByName(items, names) {
  const wanted = (Array.isArray(names) ? names : [names]).map(auditedNormalize).filter(Boolean);
  for (const item of auditedArray(items)) {
    const name = auditedNormalize(auditedName(item));
    if (!name) continue;
...
function auditedFindById(items, id) {
  if (id === null || id === undefined || id === "") return null;
  return auditedArray(items).find(item => String(auditedId(item)) === String(id)) || null;
}

async function auditedFetchDictionaries() {
  const client = createThuliumClient();
  const result = {};
  for (const [key, endpoint] of Object.entries({
    statuses: AUDITED_THULIUM_TICKET_ENDPOINTS.statuses,
    categories: AUDITED_THULIUM_TICKET_ENDPOINTS.categories,
    queues: AUDITED_THULIUM_TICKET_ENDPOINTS.queues
  })) {
    try {
...
function auditedCategoryHints(text) {
  const t = auditedNormalize(text);
  const rules = [
    { keys: ["faktura", "invoice", "paragon", "vat", "transakcja"], cats: ["faktura", "faktury", "ksiegowosc", "platnosc", "płatność", "newsletter"] },
    { keys: ["platnosc", "płatność", "paypal", "przelew", "blik", "karta", "payu"], cats: ["platnosc", "płatność", "płatności", "ksiegowosc"] },
    { keys: ["zwrot", "refund", "reklamacja", "chargeback"], cats: ["zwrot", "reklamacja", "platnosc", "płatność"] },
    { keys: ["licencja", "kod", "klucz", "license"], cats: ["licencje", "licencja", "kody"] },
    { keys: ["konto", "logowanie", "haslo", "hasło", "dostep", "dostęp"], cats: ["konto", "dostep", "dostęp", "techniczne"] },
    { keys: ["umowa", "autenti", "dokument", "podpis"], cats: ["umowa", "autenti", "dokumenty"] }
  ];
  const out = [];
  for (const rule of rules) if (rule.keys.some(k => t.includes(auditedNormalize(k)))) out.push(...rule.cats);
  return [...new Set(out)];
}

function auditedPickCategory(categories, { categoryId = null, categoryName = null, content = "", ticket = null } = {}) {
  const items = auditedArray(categories);
  if (!items.length) return { error: "CATEGORIES_NOT_LOADED", message: "Nie pobrano kategorii Thulium." };
  if (categoryId) {
    const found = auditedFindById(items, categoryId);
    return found || { error: "CATEGORY_ID_NOT_FOUND", message: `Kategoria ID ${categoryId} nie istnieje w Thulium.` };
  }
  if (categoryName) {
    const found = auditedFindByName(items, categoryName);
    return found || { error: "CATEGORY_NAME_NOT_FOUND", message: `Kategoria ${categoryName} nie istnieje w Thulium.` };
  }
  const text = [content, JSON.stringify(ticket || {})].join(" ");
  for (const hint of auditedCategoryHints(text)) {
    const found = auditedFindByName(items, hint);
    if (found) return found;
  }
  return items[0];
}

function auditedHtml(value) {
  return String(value || "")
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;")
...
function auditedAgentPayloads(content) {
  const html = auditedHtml(content);
  return [
    { body: { content } },
    { body: { text: content } },
    { body: { html } },
...
function auditedAgentFormPayloads(content) {
  const html = auditedHtml(content);
  return [
    ["body[content]", content],
    ["body[text]", content],
    ["body[html]", html],
    ["body[0][content]", content],
    ["body[0][type]", "text"],
    ["message[body][content]", content]
  ];
}

async function auditedSendReply(ticketId, content) {
  const allowed = auditedWriteAllowed("auditedSendReply");
  if (!allowed.ok) return allowed;
  const client = createThuliumClient();
  const endpoint = AUDITED_THULIUM_TICKET_ENDPOINTS.agentResponse(ticketId);
  const attempts = [];

  for (const payload of auditedAgentPayloads(content)) {
    try {
      const response = await client.post(endpoint, payload, { headers: { "Content-Type": "application/json", "Accept": "application/json", "Accept-Language": "pl" } });
      return { ok: true, senderVersion: "v1.0.0-audited", endpoint, usedPayload: payload, contentType: "application/json", data: response.data };
    } catch (error) {
      attempts.push({ endpoint, payload, contentType: "application/json", error: normalizeError(error) });
    }
  }

  for (const [key, value] of auditedAgentFormPayloads(content)) {
    const params = new URLSearchParams();
    params.set(key, value);
    try {
      const response = await client.post(endpoint, params.toString(), { headers: { "Content-Type": "application/x-www-form-urlencoded", "Accept": "application/json", "Accept-Language": "pl" } });
      return { ok: true, senderVersion: "v1.0.0-audited", endpoint, usedPayload: { [key]: value }, contentType: "application/x-www-form-urlencoded", data: response.data };
    } catch (error) {
      attempts.push({ endpoint, payload: { [key]: value }, contentType: "application/x-www-form-urlencoded", error: normalizeError(error) });
    }
  }

  return { ok: false, senderVersion: "v1.0.0-audited", error: "AGENT_RESPONSE_REJECTED_AUDITED_PAYLOADS", endpoint, firstAttemptMustBe: { body: { content } }, attempts };
}

async function auditedApplyCloseCategory(ticketId, { content = "", categoryId = null, categoryName = null, dryRun = false } = {}) {
  const allowed = dryRun ? { ok: true } : auditedWriteAllowed("auditedApplyCloseCategory");
  if (!allowed.ok) return allowed;
  const dictionaries = await auditedFetchDictionaries();
  const statuses = auditedArray(dictionaries.statuses && dictionaries.statuses.data);
  const categories = auditedArray(dictionaries.categories && dictionaries.categories.data);
  const closedStatus = auditedFindByName(statuses, ["Zamknięte", "Zamkniete", "Closed", "Close"]);
  if (!closedStatus) return { ok: false, error: "CLOSED_STATUS_NOT_FOUND", statuses: statuses.map(x => ({ id: auditedId(x), name: auditedName(x) })) };
  const category = auditedPickCategory(categories, { categoryId, categoryName, content });
  if (category && category.error) return { ok: false, error: category.error, message: category.message, availableCategories: categories.map(x => ({ id: auditedId(x), name: auditedName(x) })) };
  const update = { status_id: auditedId(closedStatus) };
  if (category && auditedId(category) !== null) { update.category_id = auditedId(category); update.ticket_category_id = auditedId(category); }
  const preview = { ok: true, endpoint: AUDITED_THULIUM_TICKET_ENDPOINTS.update(ticketId), update, proposedStatus: { id: auditedId(closedStatus), name: auditedName(closedStatus) }, proposedCategory: category ? { id: auditedId(category), name: auditedName(category) } : null };
  if (dryRun) return { ...preview, dryRun: true };
  const client = createThuliumClient();
  const response = await client.put(AUDITED_THULIUM_TICKET_ENDPOINTS.update(ticketId), update);
  return { ...preview, data: response.data };
}

async function sendFinalTicketResponseAudited({ ticketId, content, categoryId = null, categoryName = null, closeTicket = true, dryRun = false }) {
  if (!ticketId) throw new Error("Missing ticketId");
  if (!content) throw new Error("Missing content");
  const preflight = closeTicket ? await auditedApplyCloseCategory(ticketId, { content, categoryId, categoryName, dryRun: true }) : { ok: true, skipped: true };
  if (closeTicket && !preflight.ok) return { ok: false, ticketId, error: "PREFLIGHT_FAILED", message: "Nie wysyłam, bo nie udało się potwierdzić statusu/kategorii.", preflight };
  if (dryRun) return { ok: true, dryRun: true, ticketId, content, preflight };
  const sendResult = await auditedSendReply(ticketId, content);
  if (!sendResult.ok) return { ok: false, ticketId, error: "SEND_REPLY_FAILED_TICKET_NOT_CLOSED", message: "Nie wysłano odpowiedzi, więc ticket NIE został zamknięty ani zmieniony.", sendResult };
  const actionResult = closeTicket ? await auditedApplyCloseCategory(ticketId, { content, categoryId, categoryName, dryRun: false }) : { ok: true, skipped: true };
  return { ok: true, ticketId, sendResult, actionResult, senderVersion: "v1.0.0-audited", message: "Wysłano odpowiedź, a potem zamknięto ticket i ustawiono kategorię." };
}
...
async function sendTicketReplySmart(ticketId, content) {
  ...
  const endpoints = [
    ...
    `/api/tickets/${encodeURIComponent(ticketId)}/agent-responses`,
    `/api/tickets/${encodeURIComponent(ticketId)}/agent_messages`,
    `/api/tickets/${encodeURIComponent(ticketId)}/messages`,
    `/api/tickets/${encodeURIComponent(ticketId)}/message`,
    `/api/tickets/${encodeURIComponent(ticketId)}/reply`,
    `/api/tickets/${encodeURIComponent(ticketId)}/response`,
    `/api/tickets/${encodeURIComponent(ticketId)}/responses`
  ];
  ...
  const payloads = [
    ...
    { response: content },
    { content, type: "agent" },
    { message: content, type: "agent" },
    { body: content, direction: "out" },
  ...
  return {
    ok: false,
    error: "NO_WORKING_TICKET_REPLY_ENDPOINT",
    message: "Nie udało się wysłać odpowiedzi żadnym znanym endpointem Thulium. Ticket NIE został zamknięty.",
...
app.get("/health", (req, res) => {
  res.json({
    ...
    documentedAgentResponseEndpoint: AUDITED_THULIUM_TICKET_ENDPOINTS.agentResponse(':id'),
...
app.post("/api/ticket-send-final", requireExtensionKey, async (req, res) => {
  try {
    ...
    const result = await sendFinalTicketResponseAudited({
      ticketId: String(ticketId),
      content,
      categoryId,
      categoryName,
